...
import { deleteTrangNoiDungAction } from './actions'

export const dynamic = 'force-dynamic'

export default async function TrangNoiDungListPage() {
  const items = await prisma.contentPage.findMany({ orderBy: { sortOrder: 'asc' } })

  return (
    <div>
      <PageHeader
        title="Trang nội dung"
        description={`${items.length} trang • Quy định chung, chính sách bảo mật, bảo hành, đổi trả...`}
        actionHref="/admin/trang-noi-dung/new"
        actionLabel="Thêm trang"
      />
                ...
                <tr key={item.id} className="transition-colors hover:bg-admin-primary/5">
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex justify-end gap-2">
                      <Link
                        href={`/admin/trang-noi-dung/${item.id}`}
                      ...
                      <ConfirmDeleteButton
                        action={deleteTrangNoiDungAction.bind(null, item.id)}
                        confirmMessage={`Xóa trang "${item.title}"?`}
                      />
                    </div>
                  </td>
                </tr>
