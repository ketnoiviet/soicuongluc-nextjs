      ...
      <div className="grid gap-5 lg:grid-cols-3">
          ...
          <GlassCard className="p-5 md:p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input name="question" defaultValue={item?.question || ''} required className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Câu trả lời</label>
                <RichTextEditor name="answerHtml" defaultValue={item?.answerHtml} height={320} />
              </div>
            </div>
          </GlassCard>
        </div>
        ...
        <div className="space-y-5">
          ...
          <SubmitButton className="admin-gradient w-full rounded-admin-md py-2.5 text-sm font-semibold text-white shadow-[0_10px_24px_-10px_rgb(var(--admin-primary)/0.6)] transition-transform hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-60">
            {item ? 'Lưu thay đổi' : 'Thêm câu hỏi'}
          </SubmitButton>
        </div>
      </div>
    </ActionForm>
