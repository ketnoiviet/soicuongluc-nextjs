  ...
  return (
    <ActionForm action={action} className="grid gap-5 md:grid-cols-3">
        ...
        <GlassCard className="p-5 md:p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="title" defaultValue={item?.title || ''} required className={inputCls} placeholder="VD: Chất lượng đảm bảo" />
            </div>
            <div>
              <label className={labelCls}>Mô tả</label>
              <textarea name="description" defaultValue={item?.description || ''} rows={4} className={inputCls} />
            </div>
          </div>
        </GlassCard>
      </div>
      ...
      <div className="space-y-5">
        <GlassCard className="p-5 md:p-6">
          <h2 className={cardTitleCls}>Icon / hình ảnh</h2>
          <div>
            {item?.iconUrl && (
              <div className="relative mb-2 aspect-square w-20 overflow-hidden rounded-admin-sm bg-admin-text-3/10">
                <Image src={getImageUrl(item.iconUrl)} alt="" fill className="object-contain" sizes="80px" />
              </div>
            )}
            <input
              type="file"
