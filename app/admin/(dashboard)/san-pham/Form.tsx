  ...
  return (
    <ActionForm action={action}>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
          ...
          <GlassCard className="p-5 md:p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <RichTextEditor name="shortDescription" defaultValue={item?.shortDescription} height={180} maxLength={1000} />
              </div>
            </div>
          </GlassCard>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {item?.thumbnailUrl && (
                    <div>
                      ...
                      <p className="mt-1 text-center text-xs text-admin-text-3">Thumbnail</p>
                    </div>
                  )}
                  {item?.coverImageUrl && (
                    <div>
                      <div className="relative aspect-square w-full overflow-hidden rounded-admin-sm bg-admin-text-3/10">
                        <Image src={getImageUrl(item.coverImageUrl)} alt="" fill className="object-cover" sizes="200px" />
                      </div>
                      <p className="mt-1 text-center text-xs text-admin-text-3">Ảnh lớn</p>
                    </div>
                  )}
                </div>
              )}
              <div>
                <label className={labelCls}>Chọn ảnh sản phẩm</label>
                <input
                  type="file"
                  name="productImage"
                  accept="image/*"
                ...
                <p className="mt-1 text-xs text-admin-text-3">
                  Tự động tạo Thumbnail (rộng tối đa 640px) và Ảnh lớn/chi tiết (rộng tối đa 1000px), chuyển sang WebP.
                </p>
              </div>
            </div>
          </GlassCard>
        ...
        <ProductSeoBox item={item} redirects={redirects} deleteRedirectAction={deleteRedirectAction} />

        <SubmitButton className="admin-gradient rounded-admin-md py-2.5 text-sm font-semibold text-white shadow-[0_10px_24px_-10px_rgb(var(--admin-primary)/0.6)] transition-transform hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-60 lg:col-span-3">
