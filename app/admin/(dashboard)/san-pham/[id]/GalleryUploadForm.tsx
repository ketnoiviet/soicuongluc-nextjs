...
export default function GalleryUploadForm({ idSP }: { idSP: number }) {
  ...
  const handleUpload = () => {
    ...
    xhr.open('POST', `/api/admin/san-pham/${idSP}/gallery-upload`)
    ...
    xhr.onload = () => {
      ...
      // Route trả 200 kèm danh sách lỗi cho từng ảnh riêng lẻ (vd. sai định dạng) thay vì fail cả batch -
      // phải đọc body chứ không chỉ dựa vào status HTTP, nếu không admin sẽ tưởng nhầm là mọi ảnh đã lên thành công.
      try {
