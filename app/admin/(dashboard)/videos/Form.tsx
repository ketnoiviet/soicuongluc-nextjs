...
import { CONTENT_STATUSES, CONTENT_STATUS_LABELS, VIDEO_SOURCE_LABELS } from '@/lib/enums'
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          <GlassCard className="p-5 md:p-6">
            <h2 className={cardTitleCls}>Thông tin video</h2>
            <div className="space-y-4">
              <div>
                <label className={labelCls}>Tiêu đề</label>
                <input name="title" defaultValue={item?.title || ''} className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>URL video (YouTube hoặc Facebook) *</label>
                <input
                  name="videoUrl"
                  type="url"
                ...
                {item && (
                  <p className="mt-1 text-xs text-admin-text-3">
                    Nguồn hiện tại: {VIDEO_SOURCE_LABELS[item.videoSource as keyof typeof VIDEO_SOURCE_LABELS] || item.videoSource} (tự nhận dạng lại khi lưu)
                  </p>
                )}
              </div>
              <div>
        ...
        <div className="space-y-5">
          <GlassCard className="p-5 md:p-6">
            <h2 className={cardTitleCls}>Hình đại diện</h2>
            <div>
              {item?.thumbnailUrl && (
