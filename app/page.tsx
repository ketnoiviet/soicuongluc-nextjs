...
export default async function HomePage() {
  ...
  const [banners, categories, featuredProducts, latestNews] = await Promise.all([
    prisma.bannerSlide.findMany({ orderBy: { thuTu: 'asc' } }),
    prisma.sanPhamLoai.findMany({
      where: { hieuLuc: 1 },
      orderBy: { thuTu: 'asc' },
    }),
    prisma.sanPham.findMany({
      where: { hieuLuc: 1, hienThi: 1, spTieuBieu: 1 },
      orderBy: { thuTu: 'asc' },
      take: 8,
    }),
      ...
      <section className="page-section bg-light">
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
            {categories.map(cat => (
              <Link key={cat.id} href={`/san-pham/${cat.url || cat.id}`}
                style={{
                  background: '#fff',
