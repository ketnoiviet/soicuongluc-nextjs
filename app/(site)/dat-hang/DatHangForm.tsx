...
export default function DatHangForm({ categories, phone }: { categories: Category[]; phone: string }) {
    ...
    <>
      ...
      {status === 'err' && (
        <div className="alert alert-error" style={{ marginBottom: 24 }}>
          ❌ Gửi thất bại.{phone ? <> Vui lòng gọi trực tiếp: <a href={`tel:${phone}`} style={{ fontWeight: 700 }}>{phone}</a></> : ' Vui lòng thử lại sau.'}
        </div>
      )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <select className="form-control" required value={form.sanPham}
              ...
              <option value="Khác (ghi rõ trong nội dung)">Khác (ghi rõ trong nội dung)</option>
            </select>
          </div>
