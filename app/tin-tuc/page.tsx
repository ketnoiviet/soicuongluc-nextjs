...
export default async function TinTucPage({
  ...
  const [total, articles, categories] = await Promise.all([
    prisma.baiViet.count({ where: { hieuLuc: 1, hienThi: 1 } }),
    prisma.baiViet.findMany({
      where: { hieuLuc: 1, hienThi: 1 },
      orderBy: { ngay: 'desc' },
      skip: (page - 1) * pageSize,
      take: pageSize,
    }),
    prisma.baiVietLoai.findMany({ where: { hieuLuc: 1 }, orderBy: { thuTu: 'asc' } }),
  ])
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <aside style={{ width: 220, flexShrink: 0 }}>
              ...
              <ul style={{ border: '1px solid var(--border)', borderTop: 'none', borderRadius: '0 0 8px 8px', overflow: 'hidden' }}>
                ...
                {categories.map(cat => (
                  <li key={cat.id}>
                    <Link href={`/tin-tuc?loai=${cat.url}`} style={{ display: 'block', padding: '10px 16px', fontSize: 13, borderBottom: '1px solid var(--bg-gray)', color: 'var(--text-dark)' }}>
                      {cat.tenLoai}
                    </Link>
                  </li>
                ))}
