...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const article = await prisma.baiViet.findFirst({ where: { link: params.slug } })
  if (!article) return { title: 'Bài viết không tồn tại' }
  return {
    title: `${article.tieuDe} | HARIFA`,
    description: article.tomTat?.substring(0, 160) || '',
  }
}

export default async function TinTucDetailPage({ params }: Props) {
  const article = await prisma.baiViet.findFirst({
  ...
  // Tăng lượt xem
  await prisma.baiViet.update({ where: { id: article.id }, data: { soLanXem: (article.soLanXem || 0) + 1 } })

  const relatedArticles = await prisma.baiViet.findMany({
    where: { idLoai: article.idLoai, hieuLuc: 1, id: { not: article.id } },
    take: 4, orderBy: { ngay: 'desc' },
  })
      ...
      <div className="breadcrumb">
        <div className="container">
          <ol>
            ...
            {article.loai && <li><Link href="/tin-tuc">{article.loai.tenLoai}</Link></li>}
            <li style={{ maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{article.tieuDe}</li>
          </ol>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <article style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--text-light)', marginBottom: 20, flexWrap: 'wrap' }}>
                <span>📅 {formatDate(article.ngay)}</span>
                {article.tacGia && <span>✍️ {article.tacGia}</span>}
                <span>👁 {article.soLanXem} lượt xem</span>
              </div>

              {article.hinhLon && (
                <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', borderRadius: 10, overflow: 'hidden', marginBottom: 24 }}>
                  <Image src={getImageUrl(article.hinhLon)} alt={article.tieuDe || ''} fill style={{ objectFit: 'cover' }} priority />
                </div>
              )}
              ...
              <div className="rich-content" dangerouslySetInnerHTML={{ __html: article.noiDung || '' }} />

              {/* Tags */}
              {article.tags && (
                <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  <span style={{ fontSize: 12, color: 'var(--text-light)' }}>🏷 Tags: {article.tags}</span>
                </div>
              )}
              ...
              <div style={{ marginTop: 24, padding: '16px', background: 'var(--bg-light)', borderRadius: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://soicuongluc.com/tin-tuc/${article.link}`)}`}
                    target="_blank" rel="noopener"
                    style={{ background: '#3b5998', color: '#fff', padding: '6px 14px', borderRadius: 4, fontSize: 12, fontWeight: 600 }}>
                    Facebook
                  </a>
                  <a href={`https://zalo.me/share?url=${encodeURIComponent(`https://soicuongluc.com/tin-tuc/${article.link}`)}`}
                    target="_blank" rel="noopener"
